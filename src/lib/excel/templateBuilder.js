...
export async function generateWorkbook(draft) {
  ...
  wb.creator = 'Unit Flow by OnEasy';
  wb.created = new Date();
...
function buildInstructionsSheet(wb, draft) {
  ...
  title.value = `${draft.companyName || 'Company'} \u2014 Unit Flow by OnEasy`;
  title.font = { bold: true, size: 16, color: { argb: 'FF1F4E79' } };

  const subtitle = ws.getCell('B3');
  subtitle.value = `Generated by Unit Flow (OnEasy) on ${new Date().toLocaleDateString('en-IN')}`;
